...
def data(file_path):
    ...
    A=tmp
    A = A[['toanslines','upload_times','answer_rate','final_score','first_score','total_time','timeavg','scoreavg']].values
    if len(A)>0:
        ...
        A = standard(A)
        rate = Topsis(A)
        res = tmp[["user_id", "case_id", "type"]].reset_index(drop=True).join(pd.DataFrame({'rate': rate}))
    return res
...
tmp=data("DataPrepare.csv")
finalanswer=pd.concat([finalanswer,tmp])
finalanswer.to_csv('Userrate.csv',index=False,encoding="utf_8_sig")