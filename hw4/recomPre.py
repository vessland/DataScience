...
def Euclidean(user1, user2):
    ...
    for key in user1_rating.keys():
        if key in user2_rating.keys():
            i += 1
            distance += pow(float(user1_rating[key])-float(user2_rating[key]), 2)
    if i == 0:
        return float("inf")
    distance = distance / i  ##可能相同题目数不同，由于各维数量纲相同（都是题目评分），因此取平均
    return distance  ##返回值越小,相似度越大
    #return 1/(1+sqrt(distance))  ##返回值越小,相似度越大

"""计算某个用户和其他用户的相似度
"""
def top4_similar(userID):
    res = []
...
def recommend(user):
    #相似度最高的用户
    top_sim_user = top4_similar(user)[0][0]
    #相似度最高的用户的做题记录
