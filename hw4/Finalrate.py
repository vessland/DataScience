...
def data(file_path, user_id):
    global res
    tmp = pd.read_csv(file_path)
    A=tmp
    A = A[A.user_id == user_id][['toanslines','upload_times','answer_rate','final_score','first_score','total_time','timeavg','scoreavg']].values
    if len(A)>0:
        ...
        A = standard(A)
        rate = Topsis(A)
        res = tmp[tmp.user_id == user_id][["user_id", "case_id", "type"]].reset_index(drop=True).join(pd.DataFrame({'rate': rate}))
    return res
...
finalanswer=pd.DataFrame(columns=('user_id','case_id','type','rate'))
...
for userid in final:
    print("正在分析用户id为："+str(userid)+"的代码")
    tmp=data("DataPrepare.csv",userid)
    finalanswer=pd.concat([finalanswer,tmp])
finalanswer.to_csv('finalrate.csv',index=False,encoding="utf_8_sig")
